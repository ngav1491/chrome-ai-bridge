...
program
  ...
  .description('chrome-ai-bridge - Local service for communicating with Chrome extension');
    ...
    try {
      ...
      if (options.system || hasElevatedPermissions) {
        ...
        if (success) {
          ...
          console.log(colorText('  1. sudo chrome-ai-bridge register', 'yellow'));
          console.log(colorText('  2. chrome-ai-bridge register --system', 'yellow'));
          process.exit(1);
        }
      }
    } catch (error: any) {
