...
if [ "$(uname)" = "Darwin" ]; then
    LOG_DIR="${HOME}/Library/Logs/chrome-ai-bridge"
else
    LOG_DIR="${XDG_STATE_HOME:-${HOME}/.local/state}/chrome-ai-bridge/logs"
fi
...
if [ -z "${NODE_EXEC}" ]; then
    {
        ...
        echo "To fix: Set CHROME_MCP_NODE_PATH environment variable or run 'chrome-ai-bridge doctor --fix'"
    } >> "${WRAPPER_LOG}"
    exit 1
fi
